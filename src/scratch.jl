Base.@kwdef struct Scratch{MatrixTimeInt, VectorInt, VectorCharge, VectorFloat, VectorCurrent, MatrixCurrent, VectorInvTime, MatrixInvTime, VectorFloat64dt, VectorVoltage, VectorNoise}
    ...
    rrXhistory = CircularArrayBuffer(MatrixInvTime(zeros(p.LX+p.Ncells+extra, round(Int, p.PHistory / p.learn_every))))
